...
class ChartPainter extends CustomPainter {
    ...
    void _drawGrid(Canvas canvas, Size size) {
        ...
        if (style.priceLabelStyle.show) {
            ...
            final textStartX =
                chartRight + layout.yAxisGap + labelPadding.left;
        ...
        if (style.timeLabelStyle.show) {
            ...
            final textStartY =
                chartBottom + layout.xAxisGap + labelPadding.top;
            verticalLineEndY = textStartY - layout.gridToLabelGapX;
        }

        final clampedVerticalEndY =
            verticalLineEndY.clamp(chartTop, size.height);
    ...
    void _drawSingleDataPoint(Canvas canvas) {
        ...
        final pointRadius =
            lineStyle.pointRadius > 0 ? lineStyle.pointRadius : 6.0;
    ...
    void _drawPriceLabels(Canvas canvas, Size size) {
        ...
        final bgColor =
            priceLabelStyle.backgroundColor ?? style.backgroundColor;
        ...
        if (labelCount == 1) {
            ...
            textPainter.text =
            TextSpan(text: priceText, style: textStyle);
            textPainter.layout();
            ...
            final chartRight =
                mapper.paddingLeft + mapper.contentWidth;
            final labelX =
    ...
    void _drawTimeLabels(Canvas canvas, Size size) {
        ...
        final bgColor =
            timeLabelStyle.backgroundColor ?? style.backgroundColor;
        ...
        final visibleCount = candles.length;
        final labelCountToShow =
            math.min(visibleCount, labelCount);
        ...
        for (int i = 0; i < labelCountToShow; i++) {
            ...
            final dataIndex =
                mapper.viewport.startIndex + relativeIndex;
            ...
            final timeText =
                formatter.format(candle.time, context: context);

            textPainter.text =
            TextSpan(text: timeText, style: textStyle);
            textPainter.layout();
        ...
        final labelY =
            chartBottom +
                layout.xAxisGap +
                xAxisLabelPadding.top;

        for (final labelInfo in visibleLabels) {
            textPainter.text =
            TextSpan(text: labelInfo.text, style: textStyle);
            textPainter.layout();
            ...
            if (bgColor.a > 0.0) {
                final labelRect = Rect.fromLTWH(
                    ...
                    labelInfo.width +
                        xAxisLabelPadding.horizontal * 2,
                    textPainter.height +
    ...
    void _drawCurrentPriceLine(Canvas canvas, Size size, double price) {
        ...
        final isPriceUp =
            previousPrice != null ? price >= previousPrice : true;

        /// Check visibility inside scale
        final isPriceVisible =
            price >= scale.min && price <= scale.max;
        ...
        if (currentPriceStyle.showLine && isPriceVisible) {
            ...
            if (style.priceLabelStyle.show) {
                final yAxisLabelPadding =
                    style.priceLabelStyle.padding;
                final textStartX =
                    chartRight + layout.yAxisGap + yAxisLabelPadding.left;
        ...
        final labelWidth =
            textPainter.width + (labelPaddingH * 2);
        final labelHeight =
            textPainter.height + (labelPaddingV * 2);
    ...
    void _drawDashedLine(
        ...
        final gapLength =
            dashArray.length > 1 ? dashArray[1] : dashArray[0];
    ...
    void _drawRippleOnLastPoint(Canvas canvas, Size size) {
        ...
        final lastCandle = candles.last;
        final lastIndex =
            mapper.viewport.startIndex + candles.length - 1;
        ...
        final centerPointRadius =
            style.lineStyle.pointRadius > 0
                ? style.lineStyle.pointRadius
                : 4.0;
        ...
        if (pulseProgress > 0.001) {
            ...
            final distanceToRight =
                (chartRight + layout.yAxisGap) - center.dx;
            ...
            final opacity =
                math.pow(1.0 - easedProgress, 0.7).clamp(0.0, 1.0);
    ...
    void _drawCrosshair(Canvas canvas, Size size) {
        ...
        /// - Chart line / candle
        final absoluteIndex =
            mapper.viewport.startIndex + crosshairIndex!;
        ...
        final labelFontSize =
            cs.labelFontSize ?? style.labelFontSize;
        ...
        if (cs.showPriceLabel) {
            ...
            final crosshairPriceFormatter =
                PriceFormatter.crosshair();

            final priceText =
                crosshairPriceFormatter.format(candle.close);
        ...
        if (cs.showTimeLabel) {
            ...
            final crosshairTimeFormatter =
                const CrosshairTimeFormatter();

            final timeText =
                crosshairTimeFormatter.format(candle.time);
