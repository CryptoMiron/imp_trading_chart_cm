...
class _ImpChartState extends State<ImpChart>
    ...
    void didUpdateWidget(ImpChart oldWidget) {
        ...
        if (!candleCountChanged &&
            ...
            lastCandleChanged =
            currentLastCandle.close != engineLastCandle.close ||
        ...
        if (candlesChanged || lastCandleChanged) {
            ...
            final isDataReplacement =
                oldCandleCount == 0 && newCandleCount > 0;
        ...
        final wasRippleEnabled =
            oldWidget.style.rippleStyle.show &&
                oldWidget.candles.isNotEmpty;
    ...
    void _handleScaleUpdate(ScaleUpdateDetails details, Size size) {
        ...
        // ─────────────────────────────────────────────
            ...
            !widget.style.crosshairStyle.show) {
            ...
            if (candleWidth > 0) {
                final candleDelta =
                    (_accumulatedPanDelta / candleWidth).round();
    ...
    void _updateCrosshair(Offset localPosition, Size size) {
        ...
        if (absoluteIndex >= 0 && absoluteIndex < _engine.candles.length) {
            ...
            final relativeIndex =
                absoluteIndex - mapper.viewport.startIndex;

            if (relativeIndex >= 0 &&
                relativeIndex < visibleCandles.length) {
    ...
    _PaddingInfo _calculatePadding(Size size) {
        ...
        if (priceLabelStyle.show && _engine.candles.isNotEmpty) {
            ...
            for (int i = 0; i <= labelCount; i++) {
                final price =
                    scale.max - ((scale.max - scale.min) * i / labelCount);
                ...
                maxLabelWidth =
                math.max(maxLabelWidth, textPainter.width);
            }
            ...
            if (currentPriceStyle.showLabel &&
                ...
                if (currentPrice != null) {
                    ...
                    textPainter.width +
                        (currentPriceStyle.labelPaddingH * 2);
                }
            }
                ...
                builder: (context, constraints) {
                    ...
                    return GestureDetector(
                        ...
                        onScaleStart: widget.style.crosshairStyle.show
                            ? null
                            : (details) =>
                            _handleScaleStart(details, size),
                        ...
                        widget.style.crosshairStyle.show
                            ? (details) =>
                            _handleLongPressStart(details, size)
                            : null,
